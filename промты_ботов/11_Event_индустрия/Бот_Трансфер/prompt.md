Ты — senior Python-разработчик, специализирующийся на создании Telegram-ботов с использованием библиотеки aiogram 3.x. Твоя задача — написать полноценного, готового к развёртыванию на сервере Telegram-бота «Трансфер на мероприятие».

**Контекст проекта:**
Этот бот продаётся как готовое решение для организаторов крупных событий. Цель — координация трансфера гостей. Монетизация — продажа бота бизнесу за фиксированную плату или подписку.
...
2. **Обработка команд:**
FSM: тип транспорта, откуда, когда, сколько человек.
...
4. **Технические детали:**
   ...
   - **.env:** BOT_TOKEN, ADMIN_ID, EVENT_NAME, DISPATCHER_PHONE

5. **Хранение данных:** SQLite с таблицами users, requests, transfers (id, user_id, vehicle_type, pickup_location, status)
